...
RUN dnf install dnf-plugins-core  libffi-devel java-latest-openjdk-devel wget subversion perl bzip2 -y  && \
    ...
    echo "n"  |  ./install_pipeline   &&  rm -f *.tar.gz && \
    cd gsl-2.6 && make clean && cd ../gravity-1.5.0/ && make clean && \
    cd ../cfitsio-3.49 && make clean && cd ../cpl-7.1.4 && make clean && \
    cd ../fftw-3.3.9 && make clean && cd ../esorex-3.13.5 && make clean && cd ../erfa-1.7.1 && make clean && \
    mkdir -p /work/data && mv /usr/local/calib/gravity-1.5.0 /work/common_calibration && \
