...
RUN dnf install dnf-plugins-core  libffi-devel java-latest-openjdk-devel wget subversion perl bzip2 gnuplot -y  && \
    cd $HOME  && \
    wget -nv https://ftp.eso.org/pub/dfs/pipelines/instruments/gravity/gravity-kit-1.5.0-6.tar.gz && \
    ...
    rm -rf gravity-calib-1.5.0 && rm -rf cfitsio-3.49* esorex-3.13.5* fftw-3.3.9* cpl-7.1.4* erfa-1.7.1* gsl-2.6* wcslib-7.6* &&\
    cd gravity-1.5.0/ && make clean && \
    mkdir -p /work/data && ln -s /usr/local/calib/gravity-1.5.0 /work/common_calibration && \
    ...
    dnf remove java-latest-openjdk-devel subversion -y
WORKDIR /work/data
